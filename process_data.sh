...
for acq in "${ACQ[@]}";do
  for rec in "${REC[@]}";do
    ...
    if [ -e "${file}${EXT}" ]; then
      ...
      file_seg=${SUBJECT_UNDERSCORE_SESSION}_${acq}_rec-navigated_${CONTRAST}_label-SC_seg
      segment_gm_if_does_not_exist ${file}
      file_gmseg=${SUBJECT_UNDERSCORE_SESSION}_${acq}_rec-navigated_${CONTRAST}_label-GM_seg
      # Calculate WM mask
